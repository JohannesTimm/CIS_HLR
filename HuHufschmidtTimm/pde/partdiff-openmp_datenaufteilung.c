...
calculate (struct calculation_arguments const* arguments, struct calculation_results *results, struct options const* options)
{
	...
	while (term_iteration > 0)
	{
		...
		#pragma omp parallel shared(Matrix_In,Matrix_Out,fpisin_i) num_threads(12)
		{
			...
			#pragma omp for  private(i,j,star,residuum) firstprivate(fpisin,pih) reduction(+:maxresiduum)  collapse(2)
			...
			//#pragma omp parallel for default(shared) private(i,j,star,residuum) firstprivate(fpisin,pih,i_start,i_end) reduction(+:maxresiduum) num_threads(4)
			for (i = 1; i < N; i++)
